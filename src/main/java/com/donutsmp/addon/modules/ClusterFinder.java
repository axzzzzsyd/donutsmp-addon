...
public class ClusterFinder extends Module {
    ...
    private final Setting<Boolean> useSlice = sgGeneral.add(new BoolSetting.Builder()
        .name("use-slice")
        .description("Only scan a horizontal slab around the player's Y level. Works around DonutSMP anti-esp: fly at the height you want to scan.")
        .defaultValue(true)
        .build()
    );
    ...
    private final Setting<Integer> sliceHeight = sgGeneral.add(new IntSetting.Builder()
        .name("slice-height")
        .description("Vertical thickness (in blocks) of the scanned slab centered on the player's Y.")
        .defaultValue(16)
        .range(4, 256)
        .sliderRange(4, 256)
        .build()
        ...
        for (int dx = -radius; dx <= radius; dx++) {
            for (int dz = -radius; dz <= radius; dz++) {
                ...
                if (useSlice.get() && mc.player != null) {
                    ...
                    maxY = Math.min(mc.level.getMaxY(), minY + 128);
                }
